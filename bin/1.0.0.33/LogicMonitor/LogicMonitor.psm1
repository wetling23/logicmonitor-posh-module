...
Function Update-LogicMonitorAlertRulesProperties {
    <#
        ...
        .PARAMETER BlockLogging
            When this switch is included, the code will write output only to the host and will not attempt to write to the Event Log.
        .EXAMPLE
            PS C:\> Update-LogicMonitorAlertRulesProperties -AccessId <accessId> -AccessKey <accessKey> -AccountName <accountName> -AlertRuleId 6 -PropertyNames hostname,collectorSize -PropertyValues server2,small
    ...
    Foreach ($property in $PropertyNames) {
        Switch ($property) {
            {$_ -in ("deviceGroups", "devices")} {
                $propertyData.Add($_, @(($($PropertyValues[$index]).Split(","))).Trim())
