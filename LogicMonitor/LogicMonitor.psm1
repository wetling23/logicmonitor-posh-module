...
[CmdletBinding()]
    ...
    If ((($StartDate -eq $null) -and ($EndDate -ne $null)) -or (($StartDate -ne $null) -and ($EndDate -eq $null))) { #If only StartDate /or/ EndDate are provided.
    ...
    ElseIf ((($StartDate -ne $null) -and ($StartDate -notmatch $regex)) -or (($EndDate -ne $null) -and ($EndDate -notmatch $regex))) { #If StartDate or EndDate are provided, but are not in the correct format.
    ...
    ElseIf (($StartDate -eq $null) -and ($EndDate -eq $null)) { #If neither StartDate nor EndDate are provided.
...
.NOTES 
    ...
    V1.0.0.5 date: 18 August 2017
        - Changed the "AppliesTo" query filter, to 
.LINK
...
<#Export-ModuleMember -Function Add-LogicMonitorCollector, Add-LogicMonitorDevice, Add-LogicMonitorDeviceGroup, Get-LogicMonitorAuditLogs, 
    Get-LogicMonitorCollectorInstaller, Get-LogicMonitorCollectors, Get-LogicMonitorDataSources, Get-LogicMonitorDeviceGroupProperties, 
    Get-LogicMonitorDeviceGroups, Get-LogicMonitorDeviceProperties, Get-LogicMonitorDevices, Get-LogicMonitorServiceProperties, 
    Get-LogicMonitorServices, Remove-LogicMonitorCollector, Remove-LogicMonitorDevice, Remove-LogicMonitorDeviceProperties,
    Update-LogicMonitorDevice
#>