Function Remove-LogicMonitorDeviceGroup {
    <#
        ...
        .EXAMPLE
            ...
            In this example, the function will remove the device group with ID "45". Verbose output is sent to the host.
        .EXAMPLE
            PS C:\> Get-LogicMonitorDeviceGroup | ? fullpath -like "Foo/Bar*" | Remove-LogicMonitorDeviceGroup

            In this example, the function will remove the device groups named Bar* within Foo path. Note: If there are subfolders, it could potentially fail, so better sort it by desc first so the most nested groups getting removed first. 
    #>
    [CmdletBinding()]
    Param (
        [Parameter(Mandatory)]
    ...
    Process {
        ...
        If ($Id -lt 2) {
            $message = ("{0}: Please specify an Id greater than 1 (Root).")
        ...
        Try {
            Write-Host "Invoke-RestMethod -Uri $url -Method $httpVerb -Header $headers -ErrorAction Stop"
